...
app.get('/api/certificates/image/:certificateId', async (req: Request, res: Response) => {
  try {
    ...
    const query = `
      SELECT id, document_path, document_name, document_type, status
...
// Middleware аутентификации (ДОЛЖЕН БЫТЬ ОБЪЯВЛЕН ПЕРЕД ИСПОЛЬЗОВАНИЕМ!)
...
// Универсальный middleware проверки роли
const requireRole = (requiredRoleId: number) => {
  return (req: Request, res: Response, next: Function) => {
    const user = (req as any).user;
...
app.get('/api/supplier/certificates', authenticateToken, requireRole(2), async (req: Request, res: Response) => {
  try {
    ...
    const query = `
      ...
      JOIN certificate_types ct ON fc.certificate_type_id = ct.id
      LEFT JOIN users u ON fc.verified_by = u.id
      WHERE fc.supplier_id = $1
      ORDER BY 
        CASE fc.status 
          WHEN 'active' THEN 1
          WHEN 'pending' THEN 2
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const checkQuery = `
        ...
        WHERE supplier_id = $1 AND certificate_type_id = $2 AND status = 'pending'
      `;
      ...
      const checkActiveQuery = `
        SELECT id FROM farmer_certificates 
        WHERE supplier_id = $1 AND certificate_type_id = $2 AND status = 'active'
      `;
      ...
      const insertRequestQuery = `
        INSERT INTO certificate_requests (supplier_id, certificate_type_id, status)
        VALUES ($1, $2, 'pending')
        RETURNING id
      `;
      ...
      const insertCertQuery = `
        INSERT INTO farmer_certificates (
          supplier_id, certificate_type_id, certificate_number, 
          issued_by, status, document_path, document_name, document_type
        )
...
app.get('/api/certificates/download/:certificateId', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const query = `
      SELECT fc.document_path, fc.document_name, fc.supplier_id, fc.status
      FROM farmer_certificates fc
      WHERE fc.id = $1
    `;
    ...
    const filePath = path.join(__dirname, '..', certificate.document_path);
...
app.put('/api/supplier/certificates/settings', authenticateToken, requireRole(2), async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      show_certificates: result.rows[0].show_certificates
    });
  } catch (error) {
...
app.get('/api/certificates/image/:certificateId', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const query = `
      SELECT fc.document_path, fc.document_name, fc.document_type, fc.supplier_id
      FROM farmer_certificates fc
      WHERE fc.id = $1
    `;
...
app.get('/api/dimensions', authenticateToken, requireRole(2), async (req: Request, res: Response) => {
  try {
...
app.post('/api/farmer/product-card', authenticateToken, requireRole(2), async (req: Request, res: Response) => {
  try {
    ...
    const farmerId = supplierResult.rows[0].id;

    // Вызываем функцию
    const query = `
            SELECT public.create_farmer_product(
...
app.get('/api/buyer/farmers-map', authenticateToken, requireRole(3), async (req: Request, res: Response) => {
  try {
    const query = `
        ...
        FROM suppliers s
	  	INNER JOIN suppliercopies ON suppliercopies.idsupplier = s.id
	  	WHERE suppliercopies."isActual" = True
      ),
      farmer_places AS (
        SELECT 
          fd.supplier_id,
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        (SELECT bidamountwhole FROM "auctionbids" WHERE idauction = a.id ORDER BY bidamountwhole DESC LIMIT 1) as "currentBid"
      FROM auctions a
...
app.post('/api/supplier/auctions', authenticateToken, requireRole(2), async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const historyQuery = `
        INSERT INTO "auctionHistory" (idauction, changed_by, status)
        VALUES ($1, $2, $3)
      `;
...
app.put('/api/supplier/warehouse/:id/move', authenticateToken, requireRole(2), async (req: Request, res: Response) => {
  try {
    ...
    const checkQuery = `
      SELECT pc.* FROM productcopies pc
...
SELECT 
  ...
  EXTRACT(DAY FROM (CURRENT_DATE - fs.created_at)) as subscription_days,
  COUNT(DISTINCT pr.id) as product_count,
  ARRAY_AGG(DISTINCT pc.name) as product_categories,
 ARRAY_AGG(DISTINCT "namesObjects".name) FILTER (WHERE "namesObjects".name IS NOT NULL) as product_names,
  -- ДОБАВЛЯЕМ ПРОВЕРКУ НА ЭКО-СЕРТИФИКАТ (тип 1, активный)
  EXISTS (
    SELECT 1 FROM farmer_certificates fc
    WHERE fc.supplier_id = s.id 
      AND fc.certificate_type_id = 1
      AND fc.status = 'active'
      AND (fc.expiry_date IS NULL OR fc.expiry_date > CURRENT_DATE)
...
INNER JOIN places p ON sp.idplace = p.id
LEFT JOIN farmer_subscriptions fs ON fs.idcustomer = $1 AND fs.idsupplier = s.id
LEFT JOIN supplierplacesproducts spp ON sp.id = spp.idsupplierplace
LEFT JOIN products pr ON spp.idproduct = pr.id
LEFT JOIN "namesObjects" ON pr.idobject = "namesObjects".id
...
GROUP BY s.id, s.name, sc.rating, sc.description, p.id, p.address, p.kadastrnumber, p.area,
         p.boundaries, fs.id, fs.created_at
ORDER BY s.id, p.id
`;
...
app.post('/api/buyer/clustered-products', authenticateToken, requireRole(3), async (req: Request, res: Response) => {
  try {
    ...
    const subsRes = await pool.query('SELECT idsupplier FROM farmer_subscriptions WHERE idcustomer = $1', [customerId]);
  ...
  SELECT 
    ...
    (SELECT bidamountwhole FROM auctionbids WHERE idauction = a.id ORDER BY bidamountwhole DESC LIMIT 1) as auction_current_bid,
    -- ДОБАВЛЯЕМ ПРОВЕРКУ НА ЭКО-СЕРТИФИКАТ
    EXISTS (
      SELECT 1 FROM farmer_certificates fc
      WHERE fc.supplier_id = s.id 
        AND fc.certificate_type_id = 1
  ...
  FROM productcopies pc
      ...
      LEFT JOIN auctions a ON a.idproduct = pr.id 
         ...
         AND a.endtime >= NOW()
         AND NOT EXISTS (
           SELECT 1 FROM auctionhistory ah 
      ...
      WHERE pc.isactual = true
        ...
        AND (($2::text = 'all') OR ($2::text = 'wholesale' AND pc.weight >= 10) OR ($2::text = 'retail' AND pc.weight < 10))
        AND (($3::text = 'all') OR
             ($3::text = 'market' AND pc.idlocationproduct = 1) OR
             ($3::text = 'auction' AND pc.idlocationproduct = 2) OR
             ($3::text = 'warehouse' AND pc.idlocationproduct = 3))
...
app.post('/api/supplier/upload-avatar',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await pool.query('UPDATE suppliers SET avatar_url = $1 WHERE id = $2', [avatarUrl, supplierId]);
...
app.get('/api/places/image/:placeId', async (req: Request, res: Response) => {
  try {
    ...
    const placeId = req.params.placeId;
    const query = 'SELECT image_url FROM places WHERE id = $1';
...
app.get('/api/supplier/avatar/:userId', async (req: Request, res: Response) => {
  try {
    ...
    if (result.rows.length === 0 || !result.rows[0].avatar_url) {
      console.log('Нет avatar_url в БД для userId =', userId);
      return res.status(404).json({ success: false, message: 'Аватарка не найдена' });
    }

    const dbUrl = result.rows[0].avatar_url;
    const avatarPath = path.resolve(__dirname, result.rows[0].avatar_url.replace(/^\/+/, ''));
