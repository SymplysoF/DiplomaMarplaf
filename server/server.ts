...
app.post('/api/check-cadastr', authenticateToken, requireRole(2), async (req, res) => {
  try {
    ...
    const response = await fetch(
      `https://nspd.gov.ru/api/geoportal/v2/search/geoportal?thematicSearchId=1&query=${encodeURIComponent(kadastrNumber)}`,
      {
        method: 'GET',
        headers: {
          Accept:
...
}); app.get('/api/diet-tables', authenticateToken, requireRole(3), async (req, res) => {
  try {
    ...
    console.error('Error loading diet tables:', error);
    res.status(500).json({
      success: false,
      message: 'Ошибка загрузки диетических столов'
    });
  }
});

app.get('/api/diet-tables/:tableId/objects', authenticateToken, requireRole(3), async (req, res) => {
  try {
    ...
    if (!Number.isInteger(tableId) || tableId <= 0) {
      return res.status(400).json({
        success: false,
        message: 'Некорректный ID диетического стола'
      });
    }
    ...
    const result = await pool.query(
      `
      SELECT DISTINCT
        p."idObject" AS "objectId",
        no.name AS "objectName",
        COALESCE(dtp."varietyId", no."idVariety") AS "varietyId",
        v.name AS "varietyName",
        CASE
          WHEN v.name IS NOT NULL THEN p.name || ' — ' || v.name
          ELSE p.name
        END AS "fullName"
      FROM "dietTableProducts" dtp
      JOIN products p 
        ON p.id = dtp."productId"
      JOIN "namesObjects" no 
        ON no.id = p."idObject"
      LEFT JOIN varieties v 
        ON v.id = COALESCE(dtp."varietyId", no."idVariety")
      WHERE dtp."tableId" = $1
      ORDER BY "fullName"
      `,
      [tableId]
    ...
    console.error('Error loading diet table objects:', error);
    res.status(500).json({
      success: false,
      message: 'Ошибка загрузки продуктов диетического стола'
    });
  }
});
...
app.delete('/api/supplier/warehouse/:id', authenticateToken, requireRole(2), async (req: Request, res: Response) => {
  try {
    ...
    await pool.query('DELETE FROM "supplierPlacesProducts" WHERE "idProduct" = $1', [id]);
